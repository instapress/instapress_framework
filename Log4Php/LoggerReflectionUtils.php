 ...
 * @package log4php
 */
class Log4Php_LoggerReflectionUtils {
		/** the target object */
	private $obj;
	...
	public static function setPropertiesByObject($obj, $properties, $prefix) {
		$pSetter = new Log4Php_LoggerReflectionUtils($obj);
	...
	public function setProperties($properties, $prefix) {
		...
		while(list($key,) = each($properties)) {
			if(strpos($key, $prefix) === 0) {
				...
				$value = Log4Php_helpers_LoggerOptionConverter::findAndSubst($key, $properties);
				$key = substr($key, $len);
				if($key == 'layout' and ($this->obj instanceof Log4Php_LoggerAppender)) {
					continue;
				}
