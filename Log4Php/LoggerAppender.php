 ...
 * @package log4php
 */
abstract class Log4Php_LoggerAppender {
	 ...
	 * @var Log4Php_LoggerFilter
	 */
	protected $filter = null;
			
	/**
	 * Log4Php_LoggerLayout for this appender. It can be null if appender has its own layout
	 * @var Log4Php_LoggerLayout
	 */
	...
	/**
	 * @var Log4Php_LoggerLevel There is no level threshold filtering by default.
	 */
	 ...
	 *
	 * @param Log4Php_LoggerFilter $newFilter add a new Log4Php_LoggerFilter
	 */
	 ...
	 * @param Log4Php_LoggerLoggingEvent $event
	 */
	public function doAppend(Log4Php_LoggerLoggingEvent $event) {
		if($this->closed) {
			return;
		}
		...
		while($f !== null) {
			switch ($f->decide($event)) {
				case Log4Php_LoggerFilter::DENY: return;
				case Log4Php_LoggerFilter::ACCEPT: return $this->append($event);
				case Log4Php_LoggerFilter::NEUTRAL: $f = $f->getNext();
			}
		}
		$this->append($event);
	}	 
	 ...
	 * @param Log4Php_LoggerLayout $layout
	 */
...
/**
	 ...
	 *
	 * @param mixed $threshold can be a {@link Log4Php_LoggerLevel} object or a string.
	 * @see LoggerOptionConverter::toLevel()
	 */
	public function setThreshold($threshold) {
		if(is_string($threshold)) {
			$this->threshold = Log4Php_helpers_LoggerOptionConverter::toLevel($threshold, null);
		} else if($threshold instanceof Log4Php_LoggerLevel) {
	 ...
	 * @param Log4Php_LoggerLevel $level
	 ...
	 * @abstract
	 */
	abstract protected function append(Log4Php_LoggerLoggingEvent $event);
