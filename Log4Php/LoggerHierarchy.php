...
class Log4Php_LoggerHierarchy {
	...
	/** Log4Php_LoggerLevel main level threshold */
	protected $threshold;
	 ...
	 */
	public function __construct(Log4Php_LoggerRoot $root) {
		$this->root = $root;
		$this->setThreshold(Log4Php_LoggerLevel::getLevelAll());
		$this->rendererMap = new Log4Php_renderers_LoggerRendererMap();
	}
	...
	public function getLogger($name) {
		if(!isset($this->ht[$name])) {
			$this->ht[$name] = new Log4Php_Logger($name);;
	...
	public function getRootLogger() {
		if(!isset($this->root) or $this->root == null) {
			$this->root = new Log4Php_LoggerRoot();
		}
		return $this->root;
	}
	 
	/**
	 * @return Log4Php_LoggerLevel Returns the threshold Level.
	 */
	 ...
	 * @return boolean
	 */
	public function isDisabled(Log4Php_LoggerLevel $level) {
	...
	public function resetConfiguration() {
		...
		$root->setLevel(Log4Php_LoggerLevel::getLevelDebug());
		$this->setThreshold(Log4Php_LoggerLevel::getLevelAll());
		$this->shutDown();
		...
		Log4Php_LoggerAppenderPool::clear();
	}
	
	/**
	 * set a new threshold level
	 *
	 * @param Log4Php_LoggerLevel $l
	 */
	public function setThreshold(Log4Php_LoggerLevel $l) {
		if($l !== null) {
			$this->threshold = $l;
		}
	}
