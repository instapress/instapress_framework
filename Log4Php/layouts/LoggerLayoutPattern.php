 ...
 * @since 0.3 
 */
class Log4Php_layouts_LoggerLayoutPattern extends Log4Php_LoggerLayout {
	...
	public function setConversionPattern($conversionPattern) {
		...
		$patternParser = new Log4Php_helpers_LoggerPatternParser($this->pattern);
	 ...
	 */
	public function formatToArray(Log4Php_LoggerLoggingEvent $event) {
		$results = array();
		$c = $this->head;
		while ($c !== null) {
			if ( ! $c instanceOf Log4Php_helper_LoggerLiteralPatternConverter) {
				$sbuf = null;
				$c->format($sbuf, $event);
                                if($sbuf!=',')
				$results[] = $sbuf;
