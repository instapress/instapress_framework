 ...
 * @since 0.5
 */
class Log4Php_configurators_LoggerConfiguratorIni implements Log4Php_LoggerConfigurator {
	 ...
	 * log4php.appender.A2.layout=Log4Php_layouts_LoggerLayoutTTCC
	 ...
	 * @param Log4Php_LoggerHierarchy $repository the repository to apply the configuration
	 */
	public function configure(Log4Php_LoggerHierarchy $hierarchy, $url = '') {
		$properties = @parse_ini_file($url);
		if ($properties === false) {
			$error = error_get_last();
			throw new Log4Php_LoggerException("LoggerConfiguratorIni: Error parsing configuration file: ".$error['message']);
		}
	 ...
	 * @param Log4Php_LoggerHierarchy $hierarchy
	 */
	private function doConfigureProperties($properties, Log4Php_LoggerHierarchy $hierarchy) {
		$thresholdStr = @$properties[self::THRESHOLD_PREFIX];
		$hierarchy->setThreshold(Log4Php_helpers_LoggerOptionConverter::toLevel($thresholdStr, Log4Php_LoggerLevel::getLevelAll()));
	 ...
	 * @param Log4Php_LoggerHierarchy $hierarchy
	 */
	private function configureRootCategory($props, Log4Php_LoggerHierarchy $hierarchy) {
	 ...
	 * @param Log4Php_LoggerHierarchy $hierarchy
	 */
	private function parseCatsAndRenderers($props, Log4Php_LoggerHierarchy $hierarchy) {
	 ...
	 */
	private function parseAdditivityForLogger($props, Log4Php_Logger $cat, $loggerName) {
		$value = Log4Php_helpers_LoggerOptionConverter::findAndSubst(self::ADDITIVITY_PREFIX . $loggerName, $props);
		$additivity = Log4Php_helpers_LoggerOptionConverter::toBoolean($value, true);
	 ...
	 * @return Logger
	 */
	private function parseCategory($props, Log4Php_Logger $logger, $optionKey, $loggerName, $value) {
		...
		if(!(empty($value) || @$value[0] == ',')) {
			...
			if('INHERITED' == strtoupper($levelStr) || 'NULL' == strtoupper($levelStr)) {
				...
				$logger->setLevel(Log4Php_helpers_LoggerOptionConverter::toLevel($levelStr, Log4Php_LoggerLevel::getLevelDebug()));
			}
		}
	...
	private function parseAppender($props, $appenderName) {
		...
		if($appender === null) {
			...
			$appender = Log4Php_LoggerAppenderPool::getAppenderFromPool($appenderName, $appenderClass);
			if($appender === null) {
				return null;
			}
		}
		...
		if($appender->requiresLayout() ) {
			...
			$layoutClass = Log4Php_helpers_LoggerOptionConverter::substVars($layoutClass, $props);
			...
			Log4Php_LoggerReflectionUtils::setPropertiesByObject($layout, $props, $layoutPrefix . ".");
			$appender->setLayout($layout);
			
		}
		Log4Php_LoggerReflectionUtils::setPropertiesByObject($appender, $props, $prefix . ".");
		return $appender;
	}
}
