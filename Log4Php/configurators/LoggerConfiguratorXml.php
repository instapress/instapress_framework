 ...
 * @since 0.4 
 */
class Log4Php_configurators_LoggerConfiguratorXml implements Log4Php_LoggerConfigurator {
	...
	<log4php:configuration threshold="all">
		<appender name="A1" class="Log4Php_appenders_LoggerAppenderDailyFile">
			<layout class="Log4Php_layouts_LoggerLayoutSimple" />
		</appender>
		<root>
			<level value="debug" />
			<appender_ref ref="A1" />
	...
	/**
	 * @var Log4Php_LoggerFilter parsed Log4Php_LoggerFilter
	 */
	private $filter;
	 ...
	 *
	 * @param string $url
	 * @param Log4Php_LoggerHierarchy $repository
	 */
	private function doConfigure($url = '', Log4Php_LoggerHierarchy  $repository)
	{
		$xmlData = '';
		if (!empty($url))
	 ...
	 * @param Log4Php_LoggerHierarchy $repository
	 */
	private function doConfigureByString($xmlData, Log4Php_LoggerHierarchy $repository)
	{
	 ...
	 * @param Log4Php_LoggerHierarchy $repository
	 */
	private function doConfigureDefault(Log4Php_LoggerHierarchy $repository)
	{
	 ...
	 * @param string $xmlData
	 */
	private function parse($xmlData, Log4Php_LoggerHierarchy $repository)
	{
	...
	private function tagOpen($parser, $tag, $attribs)
	{
		switch ($tag) {
			...
			case self::XMLNS.':CONFIGURATION':
				...
				if (isset($attribs['THRESHOLD'])) {
					...
					$this->repository->setThreshold(
						Log4Php_helpers_LoggerOptionConverter::toLevel(
			...
			case self::XMLNS.':APPENDER':
				...
				$this->appender = Log4Php_LoggerAppenderPool::getAppenderFromPool($name, $class);
			...
			case self::XMLNS.':LAYOUT':
				$class = @$attribs['CLASS'];
				$this->layout = Log4Php_LoggerReflectionUtils::createObject($this->subst($class));
			...
			case self::XMLNS.':LOGGER':
				...
				if (!empty($loggerName)) {
					...
					if ($this->logger !== null and isset($attribs['ADDITIVITY'])) {
						$additivity = Log4Php_helpers_LoggerOptionConverter::toBoolean($this->subst($attribs['ADDITIVITY']), true);
				...
				switch (end($this->state)) {
					case self::APPENDER_STATE:
						if ($this->appender !== null) {
							Log4Php_LoggerReflectionUtils::setter($this->appender, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					case self::LAYOUT_STATE:
						if ($this->layout !== null) {
							Log4Php_LoggerReflectionUtils::setter($this->layout, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					case self::FILTER_STATE:
						if ($this->filter !== null) {
							Log4Php_LoggerReflectionUtils::setter($this->filter, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					default:
			...
			case self::XMLNS.':ROOT':
				$this->logger = Log4Php_Logger::getRootLogger();
			...
			case self::XMLNS.':APPENDER':
				if ($this->appender !== null) {
					if ($this->appender->requiresLayout() and $this->appender->getLayout() === null) {
						...
						$this->appender->setLayout(Log4Php_LoggerReflectionUtils::createObject('Log4Php_layouts_LoggerLayoutSimple'));
					}					
	...
	{
		return Log4Php_helpers_LoggerOptionConverter::substVars($value);
	}
