 ...
 * @since 0.5
 */
class Log4Php_helpers_LoggerOptionConverter {
	 ...
	 * character is present, then the default {@link Log4Php_LoggerLevel}
	 ...
	 *
	 * @param string $value
	 * @param Log4Php_LoggerLevel $defaultValue
	 * @return Log4Php_LoggerLevel a {@link Log4Php_LoggerLevel} or null
	 * @static
	 */
	public static function toLevel($value, $defaultValue) {
		...
		if($hashIndex === false) {
			if("NULL" == strtoupper($value)) {
				...
				return Log4Php_LoggerLevel::toLevel($value, $defaultValue);
			}
		}
		...
		if(class_exists($clazz)) {
			...
			if(!$result instanceof Log4Php_LoggerLevel) {
				$result = $defaultValue;
			}
		} 
		return $result;
