 ...
 * @package log4php
 */
class Log4Php_LoggerLoggingEvent {
	...
	*/
	...
	/** 
	* @var Log4Php_LoggerLocationInfo Location information for the caller.
	*/
	...
	* @param Log4Php_LoggerLevel $priority The level of this event.
	...
	*/
	public function __construct($fqcn, $logger, $priority, $message, $timeStamp = null, Exception $throwable = null) {
		$this->fqcn = $fqcn;
		if($logger instanceof Log4Php_Logger) {
			$this->logger = $logger;
		...
		if ($throwable !== null && $throwable instanceof Exception) {
			$this->throwableInfo = new Log4Php_LoggerThrowableInformation($throwable);
		}
	}
	 ...
	 * @return Log4Php_LoggerLocationInfo
	 */
				...
				while($hop !== null) {
					if(isset($hop['class'])) {
					  ...
					  $className = strtolower($hop['class']);
						if(!empty($className) and ($className == 'log4php_logger' or $className == 'log4php_loggercategory' or
							strtolower(get_parent_class($className)) == 'log4php_logger' or
							strtolower(get_parent_class($className)) == 'log4php_loggercategory')) {
...
//                                print_r($locationInfo);
			...
			$this->locationInfo = new Log4Php_LoggerLocationInfo($locationInfo, $this->fqcn);
		}
	 ...
	 * thread or even on a different machine. The {@link Log4Php_LoggerNDC::get()} method
	...
	public function getNDC() {
		if($this->ndcLookupRequired) {
			...
			$this->ndc = Log4Php_LoggerNDC::get();
		}
		return $this->ndc;
	}
	...
	public function getMDC($key) {
		return Log4Php_LoggerMDC::get($key);
	}
	...
	public function getRenderedMessage() {
		if($this->renderedMessage === null and $this->message !== null) {
			if(is_string($this->message)) {
				...
				// But in contrast to log4j, in log4php there is only have one Log4Php_LoggerHierarchy so there is
				// no need figure out which one is $this->logger part of.
				// TODO: Logger::getHierarchy() is marked @deprecated!
				$repository = Log4Php_Logger::getHierarchy();
	...
	public function getTime() {
		...
		$eventStartTime = (float)Log4Php_LoggerLoggingEvent::getStartTime();
...
Log4Php_LoggerLoggingEvent::getStartTime();
