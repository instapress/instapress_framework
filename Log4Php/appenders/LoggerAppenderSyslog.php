 ...
 */ 
class Log4Php_appenders_LoggerAppenderSyslog extends Log4Php_LoggerAppender {
	...
	public function append(Log4Php_LoggerLoggingEvent $event) {
		...
		if(!$this->dry) {
			...
			if($this->_overridePriority) {
				...
				if($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelFatal())) {
					syslog(LOG_ALERT, $message);
				} else if ($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelError())) {
					syslog(LOG_ERR, $message);		  
				} else if ($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelWarn())) {
					syslog(LOG_WARNING, $message);
				} else if ($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelInfo())) {
					syslog(LOG_INFO, $message);
				} else if ($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelDebug())) {
					syslog(LOG_DEBUG, $message);
				} else if ($level->isGreaterOrEqual(Log4Php_LoggerLevel::getLevelTrace())) {
