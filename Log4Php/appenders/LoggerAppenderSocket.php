 ...
 */ 
class Log4Php_appenders_LoggerAppenderSocket extends Log4Php_LoggerAppender {
	...
	public function activateOptions() {
		if(!$this->dry) {
			...
			if ($this->sp === false) {
				throw new Log4Php_LoggerException("Could not open socket to ".$this->getRemoteHost().":".$this->getPort().": $errstr ($errno)");
			}
		}
		if($this->getUseXml()) {
			$this->xmlLayout = Log4Php_LoggerReflectionUtils::createObject('Log4Php_layouts_LoggerLayoutXml');
	...
	public function setLocationInfo($flag) {
		$this->locationInfo = Log4Php_helpers_LoggerOptionConverter::toBoolean($flag, $this->getLocationInfo());
	} 
	...
	public function setLog4jNamespace($flag) {
		$this->log4jNamespace = Log4Php_helpers_LoggerOptionConverter::toBoolean($flag, $this->getLog4jNamespace());
	} 
	...
	public function setPort($port) {
		$port = Log4Php_helpers_LoggerOptionConverter::toInt($port, 0);
	...
	public function setTimeout($timeout) {
		$this->timeout = Log4Php_helpers_LoggerOptionConverter::toInt($timeout, $this->getTimeout());
	}
	...
	public function setUseXml($flag) {
		$this->useXml = Log4Php_helpers_LoggerOptionConverter::toBoolean($flag, $this->getUseXml());
	} 
 
	public function append(Log4Php_LoggerLoggingEvent $event) {
		if($this->sp || $this->dry) {
