 ...
 * @since 2.0
 */
class Log4Php_appenders_LoggerAppenderPDO extends Log4Php_LoggerAppender {
		...
		try {
			if($this->user === null) {
				...
				$this->db = new PDO($this->dsn, $this->user);
                            ...
                            //echo 'database credentials'.$this->dsn.'@@'.$this->user.'@@'.$this->password;
			...
			throw new Log4Php_LoggerException($e);
		}
		
		$this->layout = new Log4Php_layouts_LoggerLayoutPattern();
	...
	public function setCreateTable($flag) {
		$this->createTable = Log4Php_helpers_LoggerOptionConverter::toBoolean($flag, true);
	}
	...
	/**
	 * Sets the {@link Log4Php_layout_LoggerLayoutPattern} format strings for {@link $insertSql}.
	 *
	 * It's not necessary to change this except you have customized logging.
	 *
