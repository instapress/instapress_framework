 ...
 */
class Log4Php_appenders_LoggerAppenderMailEvent extends Log4Php_LoggerAppender {
	...
	public function activateOptions() {
		if (empty($this->layout)) {
			throw new Log4Php_LoggerException("LoggerAppenderMailEvent requires layout!");
		}
		if (empty($this->to)) {
			throw new Log4Php_LoggerException("LoggerAppenderMailEvent was initialized with empty 'from' ($this->from) or 'to' ($this->to) Adress!");
		}
		
		$sendmail_from = ini_get('sendmail_from');
		if (empty($this->from) and empty($sendmail_from)) {
			throw new Log4Php_LoggerException("LoggerAppenderMailEvent requires 'from' or on win32 at least the ini variable sendmail_from!");
		}
