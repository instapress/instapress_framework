 ...
 *
 * <p>If the level of the {@link Log4Php_LoggerLoggingEvent} is not between Min and Max
 ...
 * @since 0.6
 */
class Log4Php_filters_LoggerFilterLevelRange extends Log4Php_LoggerFilter {
	...
	public function setAcceptOnMatch($acceptOnMatch) {
		$this->acceptOnMatch = Log4Php_helpers_LoggerOptionConverter::toBoolean($acceptOnMatch, true);
	}
	...
	public function setLevelMin($l) {
		if($l instanceof Log4Php_LoggerLevel) {
			$this->levelMin = $l;
		} else {
			$this->levelMin = Log4Php_helpers_LoggerOptionConverter::toLevel($l, null);
		}
	}
	...
	public function setLevelMax($l) {
		if($l instanceof Log4Php_LoggerLevel) {
			...
			$this->levelMax = Log4Php_helpers_LoggerOptionConverter::toLevel($l, null);
		}
	}
	 ...
	 * @return integer
	 */
	public function decide(Log4Php_LoggerLoggingEvent $event) {
		$level = $event->getLevel();
		
		if($this->levelMin !== null) {
			if($level->isGreaterOrEqual($this->levelMin) == false) {
				// level of event is less than minimum
				return Log4Php_LoggerFilter::DENY;
			}
		}
		...
		if($this->acceptOnMatch) {
			...
			return Log4Php_LoggerFilter::ACCEPT;
		} else {
			// event is ok for this filter; allow later filters to have a look..
			return Log4Php_LoggerFilter::NEUTRAL;
		}
	}
}
