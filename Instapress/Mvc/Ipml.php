...
class Instapress_Mvc_Ipml
{
	...
	public function parseLoop(&$dom)
	{
		...
		foreach($loopsarr as $looptag )
		{
			...
			if (is_numeric($looptag->start) and is_numeric($looptag->stop) and is_numeric($increment))
			{
				...
				//$looptag=$this->parseIf( $looptag, $start );
				//$looptag=$this->parseHtmlTag($looptag);
				//$looptag=$this->parseRegexVars($looptag, $start );
				$looptag=$this->parseContent( $looptag, $start );
				...
				{
					$temp=$looptag;
					//$temp=$this->parseIf( $temp, $i );
					//$looptag=$this->parseHtmlTag( $looptag);
					//$looptag=$this->parseRegexVars($looptag, $i );
					$looptag=$this->parseContent( $temp, $i );
					$looptag->outertext .= $looptag->innertext;
				} // end of For loop
	...
	public function parseIf(&$dom,$counter=0)
	{
		...
		if($tco>0){
			...
			$tempDom->load($iftag->outertext, true);
			return $tempDom;
		}
		*/
		//return $dom;
 ...
 $dom->outertext=$rpls;
  // echo $dom->outertext;echo "\n\n\n\n\n\n\n\n------------------------\n\n\n\n\n\n";
	...
	$tempDom = new Instapress_Core_Html_Dom;
	    $tempDom->load($dom->outertext, true);
